...
@Slf4j
public class RaveNode extends UctNode {

    private final double[] raveValues;
    private final double[] raveVisits;
    Map<Player, List<Node>> simulationMoves;

    private final double RAVE_CONST = 300;

    public RaveNode(Node[][] state,
                    Player activePlayer,
                    Node[] validActions,
                    Player winner,
                    Integer action,
        ...
        this.raveValues = new double[validActions.length];
        this.raveVisits = new double[validActions.length];
    }

    public double[] calculateUpperConfidenceBoundForTreesWithRave() {
        double[] ucb = calculateUpperConfidenceBoundForTrees(0.75);

        double[] rave = IntStream.range(0, raveValues.length)
                .mapToDouble(i -> raveVisits[i] != 0.0 ? raveValues[i] / raveVisits[i] : 0.0)
                .toArray();

        double N = Arrays.stream(super.childVisits).sum();
        double alpha = Math.max(0, (RAVE_CONST - N) / RAVE_CONST);
        return IntStream.range(0, ucb.length)
                .mapToDouble(i -> (alpha * rave[i]) + (1 - alpha) * ucb[i])
                .toArray();
    }

    @Override
    public SelectionResult select() {
        RaveNode currentNode = this;
        Integer bestAction;
        ...
        while (true) {
            ...
            double[] uctRaveValues = currentNode.calculateUpperConfidenceBoundForTreesWithRave();
            bestAction = getArgMax(uctRaveValues);

            if (currentNode.children.containsKey(bestAction)) {
                currentNode = (RaveNode) currentNode.children.get(bestAction);
            } else {
                break;
            }
        }
    ...
    @Override
    public RaveNode expand(Game simulationEnv, Integer nextAction) {
    ...
    public void backup(Player winner) {
        ...
        while (true) {
            ...
            currentNode = (RaveNode) currentNode.parent;
            ...
            for (int i = 0; i < currentNode.validActions.length; i++) {
                ...
                currentNode.raveVisits[i]++;
                if (currentNode.activePlayer == winner) {
                    currentNode.raveValues[i]++;
                }
            }
        }
    }
